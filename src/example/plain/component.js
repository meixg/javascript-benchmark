...
childSlots['0'].childSlots = {}
childSlots['0'].childSlots[""] = function  (ctx, parentCtx, data) {
    let html = ""
    ...
    html += sanSSRResolver.getRenderer({specifier: "./component2", id: "default"}, "x-p", ctx.context)({}, {noDataOutput: true, parentCtx: ctx.parentCtx, tagName: "x-p", slots: ctx.slots.childSlots1, ComponentClass: ChildComponentClass});
    ctx.data = compData;
    return html;
};

childSlots['0'].childSlots1 = {}
childSlots['0'].childSlots1[""] = function  (ctx, parentCtx, data) {
    let html = ""
    html += "<!--s-slot-->";
...
sanSSRResolver.setRenderer("0", function  (data, ...info) {
    ...
    ctx.slots = {
        childSlots: {
            "": childSlots['0'].childSlots[""].bind(undefined, ctx)
        },
        childSlots1: {
            "": childSlots['0'].childSlots1[""].bind(undefined, ctx)
        }
    }
    for (let [key, val] of _.iterate(list)) {
        ...
        html += sanSSRResolver.getRenderer({specifier: ".", id: "1"}, "x-a", ctx.context)({}, {noDataOutput: true, parentCtx, tagName: "x-a", slots: ctx.slots.childSlots, ComponentClass: ChildComponentClass1});
    }
    html += "</div>";
    return html;
});
